    ...
    <main className="selector-root">
      ...
      <div className="selector-container">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          <button
            ...
            style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: '0.75rem', fontWeight: 700, letterSpacing: '1.5px', padding: '9px 20px', borderRadius: 10, background: 'transparent', border: '1px solid rgba(0,0,0,0.12)', color: '#4a5568', cursor: 'pointer', transition: 'all 0.2s' }}
          >
            🚪 LOGOUT
          </button>
        </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── BACKGROUND ── */
        .selector-root { min-height: 100vh; background: #F0F4F8; display: flex; align-items: center; justify-content: center; font-family: 'DM Sans', sans-serif; position: relative; overflow: hidden; }

        /* grid lines flipped to dark-on-light */
        .selector-bg { position: absolute; inset: 0; pointer-events: none; background: radial-gradient(ellipse 70% 55% at 15% 25%, rgba(244,121,32,0.12) 0%, transparent 60%), radial-gradient(ellipse 60% 50% at 85% 75%, rgba(0,48,135,0.15) 0%, transparent 60%), radial-gradient(ellipse 50% 40% at 50% 90%, rgba(14,165,160,0.08) 0%, transparent 55%), repeating-linear-gradient(0deg, transparent, transparent 79px, rgba(0,0,0,0.04) 80px), repeating-linear-gradient(90deg, transparent, transparent 79px, rgba(0,0,0,0.04) 80px); }
        ...
        /* ── HEADER TEXT ── */
        .iocl-name { font-family: 'Rajdhani', sans-serif; font-size: 1.6rem; font-weight: 700; color: #0d1b2e; letter-spacing: -0.5px; }
        .iocl-tagline { font-family: 'Rajdhani', sans-serif; font-size: 0.62rem; font-weight: 600; letter-spacing: 3px; color: var(--iocl-orange); }
        .header-title { font-family: 'Rajdhani', sans-serif; font-size: clamp(1.8rem, 4vw, 2.8rem); font-weight: 700; color: #0d1b2e; letter-spacing: -1px; line-height: 1.05; }
        .header-sub { font-size: 0.85rem; color: #4a5568; max-width: 480px; line-height: 1.65; }

        .cards-row { display: flex; align-items: stretch; gap: 14px; width: 100%; }

        /* ── CARDS — white bg, real shadow, dark border ── */
        .machine-card { position: relative; flex: 1; background: #FFFFFF; border: 1px solid rgba(0,0,0,0.08); border-radius: 20px; padding: 0; cursor: pointer; text-align: left; display: flex; flex-direction: column; transition: all 0.3s ease; overflow: hidden; min-width: 0; box-shadow: 0 2px 12px rgba(0,0,0,0.07); }
        ...
        .motor-card:hover { transform: translateY(-5px); border-color: rgba(244,121,32,0.35); background: #fff8f4; box-shadow: 0 20px 48px rgba(244,121,32,0.12); }
        .pump-card:hover { transform: translateY(-5px); border-color: rgba(0,48,135,0.3); background: #f4f7ff; box-shadow: 0 20px 48px rgba(0,48,135,0.1); }
        .compressor-card:hover { transform: translateY(-5px); border-color: rgba(14,165,160,0.35); background: #f2fafa; box-shadow: 0 20px 48px rgba(14,165,160,0.1); }
        .turbine-card:hover { transform: translateY(-5px); border-color: rgba(124,58,237,0.3); background: #f8f5ff; box-shadow: 0 20px 48px rgba(124,58,237,0.1); }
        ...
        .pump-icon-bg { background: rgba(0,48,135,0.1); color: #2c5db3; }
        .compressor-icon-bg { background: rgba(14,165,160,0.12); color: var(--iocl-teal); }
        .turbine-icon-bg { background: rgba(124,58,237,0.12); color: #7c3aed; }
        ...
        .motor-label { background: rgba(244,121,32,0.1); color: #c45e0a; border: 1px solid rgba(244,121,32,0.25); }
        .pump-label { background: rgba(0,48,135,0.08); color: #1a3f8f; border: 1px solid rgba(0,48,135,0.2); }
        .compressor-label { background: rgba(14,165,160,0.1); color: #0a7a76; border: 1px solid rgba(14,165,160,0.25); }
        .turbine-label { background: rgba(124,58,237,0.1); color: #6025c0; border: 1px solid rgba(124,58,237,0.25); }

        /* ── CARD CONTENT ── */
        .card-content { flex: 1; padding: 16px 20px; }
        .card-title { font-family: 'Rajdhani', sans-serif; font-size: 1.3rem; font-weight: 700; color: #1a2744; margin-bottom: 8px; letter-spacing: -0.3px; }
        .card-desc { font-size: 0.8rem; color: #4a5568; line-height: 1.6; margin-bottom: 14px; }
        ...
        .motor-tag { background: rgba(244,121,32,0.08); border: 1px solid rgba(244,121,32,0.2); color: #c45e0a; }
        .pump-tag { background: rgba(0,48,135,0.07); border: 1px solid rgba(0,48,135,0.18); color: #1a3f8f; }
        .compressor-tag { background: rgba(14,165,160,0.08); border: 1px solid rgba(14,165,160,0.2); color: #0a7a76; }
        .turbine-tag { background: rgba(124,58,237,0.08); border: 1px solid rgba(124,58,237,0.2); color: #6025c0; }

        .motor-card:hover .motor-tag { border-color: rgba(244,121,32,0.4); color: var(--iocl-orange); }
        .pump-card:hover .pump-tag { border-color: rgba(0,48,135,0.35); color: #003087; }
        .compressor-card:hover .compressor-tag { border-color: rgba(14,165,160,0.4); color: var(--iocl-teal); }
        .turbine-card:hover .turbine-tag { border-color: rgba(124,58,237,0.4); color: #7c3aed; }

        /* ── CARD FOOTER ── */
        .card-footer { display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; border-top: 1px solid rgba(0,0,0,0.07); font-family: 'Rajdhani', sans-serif; font-size: 0.8rem; font-weight: 600; letter-spacing: 0.5px; transition: all 0.3s ease; }
        .motor-footer { color: rgba(196,94,10,0.7); }
        .pump-footer { color: rgba(26,63,143,0.7); }
        .compressor-footer { color: rgba(10,122,118,0.7); }
        .turbine-footer { color: rgba(96,37,192,0.7); }
        .motor-card:hover .motor-footer { color: var(--iocl-orange); }
        .pump-card:hover .pump-footer { color: #003087; }
        .compressor-card:hover .compressor-footer { color: var(--iocl-teal); }
        .turbine-card:hover .turbine-footer { color: #7c3aed; }

        .arrow { transition: transform 0.3s ease; font-size: 1rem; }
        .machine-card:hover .arrow { transform: translateX(5px); }

        /* ── FOOTER ── */
        .selector-footer { font-family: 'Rajdhani', sans-serif; font-size: 0.7rem; font-weight: 500; color: #8a9ab5; letter-spacing: 1px; }
