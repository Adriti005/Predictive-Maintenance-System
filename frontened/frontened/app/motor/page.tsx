...
export default function MotorPage({ onBack }: Props) {
  ...
  const statusColor =
    result?.status === 'HEALTHY'  ? '#0a7a76' :
    result?.status === 'FAULTY'   ? '#b45309' : '#c45e0a'

  const statusBg =
    result?.status === 'HEALTHY'  ? 'rgba(14,165,160,0.08)'  :
    result?.status === 'FAULTY'   ? 'rgba(245,158,11,0.08)'  : 'rgba(244,121,32,0.08)'
