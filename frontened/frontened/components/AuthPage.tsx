...
export default function AuthPage({ onLogin }: { onLogin: (u: User) => void }) {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F0F4F8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", position: 'relative', overflow: 'hidden' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .auth-input { width: 100%; padding: 11px 14px; border-radius: 10px; border: 1px solid rgba(0,0,0,0.10); background: #FFFFFF; color: #1a2744; font-family: 'DM Sans', sans-serif; font-size: 14px; outline: none; transition: border-color 0.2s; box-shadow: 0 1px 4px rgba(0,0,0,0.05); }
        .auth-input:focus { border-color: rgba(244,121,32,0.5); }
        .auth-input option { background: #FFFFFF; color: #1a2744; }
        ...
        .auth-tab:not(.active) { background: transparent; color: #8a9ab5; }
        .auth-tab:not(.active):hover { color: #1a2744; }
        ...
        .auth-err { color: #c45e0a; font-size: 11px; margin-top: 4px; font-family: 'Rajdhani', sans-serif; font-weight: 600; }
        .auth-label { font-family: 'Rajdhani', sans-serif; font-size: 11px; font-weight: 700; color: #1a3f8f; letter-spacing: 1.5px; display: block; margin-bottom: 6px; }
      `}</style>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 460, padding: 20 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontFamily: "'Rajdhani', sans-serif", color: '#8a9ab5', fontSize: 11, letterSpacing: 1.5, fontWeight: 600 }}>GUWAHATI REFINERY — FAULT DETECTION SYSTEM</div>
        </div>

        <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 20, padding: 28, position: 'relative', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ display: 'flex', background: 'rgba(0,0,0,0.04)', borderRadius: 10, padding: 4, marginBottom: 22 }}>
          ...
          {msg && (
            <div style={{ background: 'rgba(10,122,118,0.08)', border: '1px solid rgba(14,165,160,0.25)', color: '#0a7a76', borderRadius: 10, padding: '10px 14px', fontSize: 12, marginBottom: 16, fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, letterSpacing: 0.5 }}>{msg}</div>
          )}
          ...
          {mode === 'register' && (
            <>
              <div style={{ marginBottom: 14 }}>
                ...
                <input className="auth-input" placeholder="e.g. Rajesh Kumar Sharma" value={form.name} onChange={e => f('name', e.target.value)} autoComplete="off" />
                {errors.name && <div className="auth-err">⚠ {errors.name}</div>}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                <div>
                  <label className="auth-label">EMPLOYEE ID</label>
                  <input className="auth-input" placeholder="IOCL-2024-XXXX" value={form.empId} onChange={e => f('empId', e.target.value)} autoComplete="off" />
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ cursor: 'pointer', color: '#8a9ab5', fontSize: 11, fontFamily: "'Rajdhani', sans-serif", fontWeight: 600 }} onClick={() => setShowPwd(p => !p)}>{showPwd ? '🙈 HIDE' : '👁 SHOW'}</span>
            </div>
          ...
          {mode === 'register' && (
            <>
              <div style={{ marginBottom: 14 }}>
                ...
                <input className="auth-input" type={showPwd ? 'text' : 'password'} placeholder="Re-enter password" value={form.confirm} onChange={e => { f('confirm', e.target.value); if (form.password === e.target.value) setErrors(prev => ({ ...prev, confirm: '' })) }} autoComplete="new-password" />
                {errors.confirm && <div className="auth-err">⚠ {errors.confirm}</div>}
              </div>
              <div style={{ background: 'rgba(0,48,135,0.06)', border: '1px solid rgba(0,48,135,0.18)', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 11, color: '#1a3f8f', fontFamily: "'Rajdhani', sans-serif", fontWeight: 600, letterSpacing: 0.5 }}>
