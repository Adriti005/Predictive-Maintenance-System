    ...
    try:
        ...
        if email:
            cur.execute("""SELECT id,prediction,confidence,risk_level,created_at,
                rpm,torque_nm,tool_wear_min,product_type,air_temp_k,proc_temp_k
                FROM motor_predictions
        ...
        return jsonify([{"id":r[0],"prediction":r[1],"confidence":r[2],"risk_level":r[3],
            "created_at":str(r[4]),"rpm":r[5],"torque":r[6],"tool_wear":r[7],
            "product_type":r[8],"air_temp":r[9],"proc_temp":r[10]} for r in rows])
    except Exception as e:
        return jsonify([])
...
def pump_history():
    try:
        ...
        return jsonify([{"id":r[0],"prediction":r[1],"confidence":r[2],"risk_level":r[3],
            "created_at":str(r[4]),
            "sensor00":r[5],"sensor02":r[6],"sensor03":r[7],"sensor04":r[8],
            "sensor05":r[9],"sensor06":r[10],"sensor07":r[11],"sensor08":r[12],
            "sensor09":r[13],"sensor10":r[14],"sensor11":r[15],"sensor12":r[16]} for r in rows])
    except Exception as e:
        return jsonify([])
...
def compressor_history():
    try:
        ...
        return jsonify([{"id":r[0],"prediction":r[1],"confidence":r[2],"risk_level":r[3],
            "created_at":str(r[4]),
            "rpm":r[5],"power":r[6],"torque":r[7],"pressure":r[8],"airflow":r[9],
            "noise":r[10],"temp":r[11],"gaccx":r[12],"gaccy":r[13],"gaccz":r[14],
            "haccx":r[15],"haccy":r[16],"haccz":r[17],"bearings":r[18]} for r in rows])
    except Exception as e:
        return jsonify([])
...
def turbine_history():
    try:
        ...
        rows = cur.fetchall(); conn.close()
        return jsonify([{"id":r[0],"prediction":r[1],"confidence":r[2],"risk_level":r[3],
            "created_at":str(r[4]),"rpm":r[5],"temp":r[6],"pressure":r[7],
            "vibration":r[8],"power":r[9]} for r in rows])
    except Exception as e:
        return jsonify([])
...
def streak():
    try:
        ...
        for machine, table, fault_statuses in [
            ...
            ("turbine",    "turbine_predictions",     "('FAULT',)"),
        ]:
...
def passport_turbine():
    try:
        conn = get_connection(); cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM turbine_predictions")
        total = int(cur.fetchone()[0] or 0)
        cur.execute("SELECT COUNT(*) FROM turbine_predictions WHERE prediction = 'FAULT'")
        faults = int(cur.fetchone()[0] or 0)
        cur.execute("""SELECT created_at FROM turbine_predictions
            WHERE prediction = 'FAULT'
            ORDER BY created_at DESC LIMIT 1""")
        row = cur.fetchone()
        if row and row[0]:
            diff = datetime.now() - row[0].replace(tzinfo=None)
        ...
        cur.execute("""
            SELECT DATE(created_at) as day,
                   COUNT(*) as total,
                   SUM(CASE WHEN prediction = 'FAULT' THEN 1 ELSE 0 END) as faults
            FROM turbine_predictions
            WHERE created_at >= NOW() - INTERVAL 30 DAY
            GROUP BY DATE(created_at)
            ORDER BY day
...
def alerts_auto():
    try:
        ...
        for machine, table, fault_col in [
            ...
            for r in cur.fetchall():
                results.append({
                    "id": r[0], "machine": machine, "prediction": r[1],
                    "confidence": float(r[2]) if r[2] else 0,
                    "risk_level": r[3], "created_at": str(r[4]), "isAuto": True
                })
        conn.close()
        results.sort(key=lambda x: x['created_at'], reverse=True)
...
# ─── REFINERY PULSE FAULT TREND ───────────────────────────────────────────────

@app.route("/refinery/fault-trend")
def refinery_fault_trend():
    try:
        conn = get_connection(); cur = conn.cursor()
        results = {}
        for machine, table, fault_statuses in [
            ...
            cur.execute(f"""
                SELECT HOUR(created_at) as hr, COUNT(*) as faults
                FROM {table}
                WHERE user_email = 'system.auto@iocl.co.in'
                AND DATE(created_at) = CURDATE()
                AND prediction IN {fault_statuses}
                GROUP BY HOUR(created_at)
                ORDER BY hr
            """)
            rows = cur.fetchall()
            hour_map = {int(r[0]): int(r[1]) for r in rows}
            results[machine] = [{"hour": f"{h:02d}:00", "faults": hour_map.get(h, 0)} for h in range(24)]
        conn.close()
        return jsonify(results)
    except Exception as e:
        return jsonify({
            "motor":      [{"hour": f"{h:02d}:00", "faults": 0} for h in range(24)],
            "pump":       [{"hour": f"{h:02d}:00", "faults": 0} for h in range(24)],
            "compressor": [{"hour": f"{h:02d}:00", "faults": 0} for h in range(24)],
            "turbine":    [{"hour": f"{h:02d}:00", "faults": 0} for h in range(24)],
        })
